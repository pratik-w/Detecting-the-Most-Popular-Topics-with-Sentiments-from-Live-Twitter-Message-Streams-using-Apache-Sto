...
public class LossyNEBolt extends BaseRichBolt{
	...
	public void lossy_counting(String NamedEntity, int sentiment)
	{
		if(number_of_elements<bucket_size) {
			if(!window.containsKey(NamedEntity)) {
				LossyObj d = new LossyObj();
				d.delta = curr_bucket-1;
				d.sentimentScore = sentiment;
				d.element = NamedEntity;
				window.put(NamedEntity, d);
			}
			else {
			 LossyObj d = window.get(NamedEntity);
				d.freq+=1;
				window.put(NamedEntity, d);
			}
	...
	public void Delete()
	{
		for(String NamedEntity: window.keySet()) {
			LossyObj d = window.get(NamedEntity);
			double freqandDelta = d.freq + d.delta;
			if(freqandDelta <= curr_bucket) {
				window.remove(NamedEntity);
			}
		}
	}
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("NamedEntity", "count","lossy_count", "sentimentScore"));
	}
